...
df_color = pd.read_csv('for_map.csv',header=0)
A_list = list(graph.nodes())
S_list = []
for a in A_list:
    b = 'bus_' + str(a)
    S_list.append(df_color.loc[df_color['buses']==b,'stds'].values[0])
...
nx.draw_networkx_nodes(G = graph, pos = pos, nodelist = graph.nodes(), 
                        node_color = S_list, cmap='coolwarm', alpha = 1, node_size = 250)
nx.draw_networkx_edges(G = graph, pos = pos, edge_color='k',
                        alpha=0.6, width = 1,arrows = False)
