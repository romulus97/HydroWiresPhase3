        ...
        for UC in UC_TREATMENTS:
            ...
            for T_p in line_limit_scaling:
                ...
                for BA_hurd in BA_hurdle_scaling:
                    ...
                    path=str(Path.cwd()) + str(Path('/Simulation_folders/PERFECT_' + str(y) + '_' + str(NN) + UC + '_' + str(T_p) + '_' + str(BA_hurd)))
                    ...
                    T_N = np.zeros((351*len(buses),337))
                    ...
                    for b in BAs_short: # change to select BA, year, etc.
                        ...
                        BA_index = BAs_short.index(b)
                        
                        filename = '../Cornell_forecast_data/load/baseline_351by337/load_baseline_' + b + '_' + str(y) + '.parquet'        
                        
                        # read in load data - initially, these are 351x337 matrices, one for each BA, each year
                        df_BA_load = pd.read_parquet(filename)
                        
                        df_BA_load['BA'] = [b]*len(df_BA_load)
                        
                        if BA_index < 1:
                            
                            combined_BA_load = df_BA_load.copy(deep=True)
                        
                        else:
                        
                            combined_BA_load = pd.concat([combined_BA_load,df_BA_load])
                    ...
                    for i in range(0,len(df_selected)):
                        ...
                        C = combined_BA_load.loc[combined_BA_load['BA']==abbr].values[:,:337]
                        
                        # not sure what second floating scalar is...
                        T_N[i*351:i*351+351,:] = T_N[i*351:i*351+351,:] + C*weight
                    
                        bn = buses[i] = 'bus_' + str(buses[i])
                        bus_names = [bn]*351
                        
                        T_bus_names = T_bus_names + bus_names
                        
                    
                    df_C = pd.DataFrame(T_N)
                    df_C.columns = df_C.columns.map(str)
                    df_C['bus'] = T_bus_names
                    df_C.to_parquet('nodal_load.parquet',index=False)   
                    
                    copy('nodal_load.parquet',path)
                    ...
                    for node in nodes:
                        ...
                        filename = '../Cornell_forecast_data/wind/baseline_351by337/wind_baseline_' + str(n) + '_' + str(y) + '.parquet'        

                        try: 
                            
                            df_nodal_wind = pd.read_parquet(filename)
                        ...
                        if l_nodes.index(node) < 1:
                            ...
                            df_C = df_nodal_wind.copy(deep=True)
                    ...
                    for node in nodes:
                        ...
                        filename = '../Cornell_forecast_data/solar/baseline_351by337/solar_baseline_' + str(n) + '_' + str(y) + '.parquet'        

                        try: 
                            
                            df_nodal_solar = pd.read_parquet(filename)
